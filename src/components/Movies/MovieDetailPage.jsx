	...
	useEffect(() => {
		!!currentUser &&
			...
			dbUsers
				...
				.get()
				.then((doc) => {
					doc.data().wishlist.includes(movie.id) ? setWishlist(true) : setWishlist(false)
					doc.data().ownDVD.includes(movie.id) ? setOwnDVD(true) : setOwnDVD(false)
		...
		dbUsers
			...
			.then((doc) => {
				dbUsers
					.doc(currentUser.uid)
					.update({
						wishlist: wishlist ? doc.data().wishlist.filter((item) => item !== movie.id) : [movie.id, ...doc.data().wishlist],
					})
					.then(() => setWishlist((prev) => !prev))
		...
		dbUsers
			...
			.then((doc) => {
				dbUsers
					.doc(currentUser.uid)
					.update({
						wishlist: ownAnyDisk ? doc.data().wishlist : doc.data().wishlist.filter((item) => item !== movie.id),
						ownDVD: ownDVD ? doc.data().ownDVD.filter((item) => item !== movie.id) : [movie.id, ...doc.data().ownDVD],
					})
					.then(() => {
						setOwnDVD((prev) => !prev)
						setWishlist(false)
		...
		dbUsers
			...
			.then((doc) => {
				dbUsers
					...
					.update({
						wishlist: ownAnyDisk ? doc.data().wishlist : doc.data().wishlist.filter((item) => item !== movie.id),
						ownBD: ownBD ? doc.data().ownBD.filter((item) => item !== movie.id) : [movie.id, ...doc.data().ownBD],
					})
					.then(() => {
						setOwnBD((prev) => !prev)
						setWishlist(false)
		...
		dbUsers
			...
			.then((doc) => {
				dbUsers
					...
					.update({
						wishlist: ownAnyDisk ? doc.data().wishlist : doc.data().wishlist.filter((item) => item !== movie.id),
						ownUBD: ownUBD ? doc.data().ownUBD.filter((item) => item !== movie.id) : [movie.id, ...doc.data().ownUBD],
					})
					.then(() => {
						setOwnUBD((prev) => !prev)
						setWishlist(false)
				...
				<PageWrapper backdrop={!!movie.backdrop_path && `https://image.tmdb.org/t/p/w780${movie.backdrop_path}`}>
					...
					{!!currentUser && (
						<>
							...
							<div className='addTo'>
								<button className='button buttonCircle' disabled={ownAnyDisk} onClick={toggleWish} style={{ backgroundColor: ownAnyDisk ? '#363e47' : wishlist && '#ea903b', color: wishlist && 'black', opacity: ownAnyDisk && 0.4 }}>
								...
								<button className='button buttonCircle' onClick={toggleDVD} style={{ backgroundColor: ownDVD && '#ea903b', fill: ownDVD ? 'black' : isDarkTheme ? '#FAFAFA' : 'black' }}>
								...
								<button className='button buttonCircle' onClick={toggleBD} style={{ backgroundColor: ownBD && '#ea903b', fill: ownBD ? 'black' : '#0095D5' }}>
								...
								<button className='button buttonCircle' onClick={toggleUBD} style={{ backgroundColor: ownUBD && '#ea903b', fill: ownUBD ? 'black' : '#0096D6' }}>
